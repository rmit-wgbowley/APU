...
<p><em>Figure 1: First Harmonic Approximation output showing transfer gain (linear) vs frequency (log) for the LLC resonant tank, plotted for varying quality factor (Qe) and inductance ratio (Ln) (Lr/Lm).</em></p></div>
...
This analytical model is written in `Python` and uses `picounits` for parameter loading and unit validation. It models the transfer gain vs input voltage using the equations described in application note `AN2450` by ST.
